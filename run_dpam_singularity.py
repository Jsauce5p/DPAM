...
def run_singularity_container(image_name, databases_dir, input_dir, dataset, threads, log_file, mode):
    ...
    if mode =='ai':
        exec_command = (
            f"singularity exec  --fakeroot --bind {databases_dir}:/mnt/databases:ro "
            f"--bind {input_dir}:{wdir}:rw {image_name} "
            f"/bin/bash -c 'cd {wdir};run_dpam_ai.py {dataset} {threads}'"
        )
...
if __name__ == "__main__":
    ...
    db_flag = 1 #check_databases(args.databases_dir)
    if db_flag == 0:
    ...
    run_singularity_container(args.image_name,databases_dir, input_dir, args.dataset, args.threads,log_file, mode)
